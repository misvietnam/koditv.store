...
mysettings = xbmcaddon.Addon(id='plugin.video.movihd.net')
...
def addDir(name, url, mode, iconimage,edit,inum,gname):
    ...
    contextmenuitems = []
    if edit:
        contextmenuitems.append(('[COLOR yellow]Add Season To Library[/COLOR]','XBMC.Container.Update(%s?url=%s&gname=%s&mode=6)'%('plugin://plugin.video.movihd.net',urllib.quote_plus(url),urllib.quote_plus(gname))))
...
def addLink(name,url,mode,iconimage,gname):
    ...
    contextmenuitems = []
    contextmenuitems.append(('Download','XBMC.Container.Update(%s?url=%s&gname=%s&mode=10)'%('plugin://plugin.video.movihd.net',urllib.quote_plus(url),urllib.quote_plus(gname))))
    contextmenuitems.append(('[COLOR yellow]Add To Library[/COLOR]','XBMC.Container.Update(%s?url=%s&gname=%s&mode=5)'%('plugin://plugin.video.movihd.net',urllib.quote_plus(url),urllib.quote_plus(gname))))
    liz.addContextMenuItems(contextmenuitems,replaceItems=False)
